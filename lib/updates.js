...
// a delegate should be deleted
exports.del_delegate = function (doc, req) {
    // remove the document with the _delegate
    db.current().removeDoc(doc, function (err, resp) {});
    
    // redirect user to list with delegates
    return [null, utils.redirect(req, '/delegates')];
};
...
exports.add_delegate = function (doc, req) {
    ...
    if (form.isValid()) {
        ...
        return [null, {content: content, title: 'Add new delegate'}];
    }
};
...
exports.change_delegate = function (doc, req) {
    ...
    if (form.isValid()) {
        ...
        return [form.values, 
		utils.redirect(req, '/' + encodeURIComponent(form.values._id))];
    } else {
        // the form is not valid, so render it again with error messages
