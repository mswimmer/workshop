...
duality_events.on('afterResponse', function(x,y) {
    ...
    db.current().getView(settings.name, "c_dinner_meals", function(err, data) {
	if (data.rows.length > 0)
	    $("#dinner_meals").html("<span title=\"Number of dinner meals including spouses\" class=\"tooltip\">Dinners: " + data.rows[0].value + "</span>");
    });
});
